...
- Subscriptions persist to `~/.snsclaw/webhook_subscriptions.json`

## How It Works

1. `the agent webhook subscribe` writes to `~/.snsclaw/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.snsclaw/logs/gateway.log | tail -20`
