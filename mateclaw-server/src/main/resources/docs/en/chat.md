...
::: tip Added in 1.3.0
When the agent's primary model is text-only (e.g. `deepseek-chat`, `kimi-k2`), uploading an image no longer breaks. The runtime auto-routes through a sidecar. See [issue #87](https://lab.sns.app/SnSclaw/issues/87).
:::
...
::: tip Added in 1.4.0
The model selector in the chat header now binds a model **to the conversation**, not as a global switch. See [issue #150](https://lab.sns.app/SnSclaw/issues/150).
:::
...
::: tip Added in 1.4.0
The conversation sidebar grew from a plain history list into an actionable operations panel. See [issue #144](https://lab.sns.app/SnSclaw/issues/144).
:::
