...
去 [GitHub Releases](https://lab.sns.app/SnSclaw/releases) 拿最新安装包。
...
- **还是不行**——去 [GitHub Issues](https://lab.sns.app/SnSclaw/issues) 开一个 Issue，把 `app.log` 的尾巴贴上。我们真的会看。
...
- **从源码跑**——`snsclaw-server/` 里 `mvn spring-boot:run`，`snsclaw-ui/` 里 `npm run dev`。细节在 [贡献指南](./contributing)。
