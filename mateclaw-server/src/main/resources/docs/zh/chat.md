...
::: tip 1.3.0 新增
当 Agent 配的主模型是纯文本模型（如 `deepseek-chat` / `kimi-k2`），上传图片不再"系统无法工作"，而是自动走旁路（sidecar）。详见 [issue #87](https://github.com/sns/SnSclaw/issues/87)。
:::
...
::: tip 1.4.0 新增
聊天顶栏的模型选择器现在把模型**绑定在会话上**，而不是全局开关。详见 [issue #150](https://github.com/sns/SnSclaw/issues/150)。
:::
...
::: tip 1.4.0 新增
会话侧栏从一条单纯的历史列表，升级成了一个可操作的运营面板。详见 [issue #144](https://github.com/sns/SnSclaw/issues/144)。
:::
