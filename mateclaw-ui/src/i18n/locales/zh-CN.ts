export default {
  ...
  settings: {
    ...
    model: {
      ...
      claudeCodeOauthRevokeHint: '请在 Claude Code 客户端中退出登录。SnSclaw 不会修改 Claude Code 的本地凭据。',
    ...
    hints: {
      ...
      workspaceStorageRootPlaceholder: '留空使用服务端默认位置，例如 /data/snsclaw/workspace',
    ...
    localTools: {
      desktopOnly: '本地工具仅在桌面客户端内可用。请在 SnSclaw 桌面应用中打开此页面进行管理。',
      enableLabel: '启用本地工具',
  ...
  security: {
    ...
    toolGuard: {
      ...
      exportBtn: '导出 JSON',
      importBtn: '导入 JSON',
      exportFileName: 'snsclaw-guard-rules-{date}.json',
  ...
  acp: {
    ...
    env: {
      ...
      hints: {
        ...
        dashscope: '阿里云 DashScope key — 跟 SnSclaw .env 里的 DASHSCOPE_API_KEY 同一个值',
  ...
  plugins: {
    ...
    noDescription: '暂无描述',
    emptyTitle: '暂无已安装插件',
    emptyHint: '将插件 JAR 文件放入 ~/.snsclaw/plugins/ 目录后重启服务',
    loading: '加载插件中...',
    loadFailed: '加载插件失败',
    enabled: '{name} 已启用',
    disabled: '{name} 已禁用',
  ...
  login: {
    ...
    heroDesc: 'SnSclaw 的重点不是把更多能力堆进聊天框，而是把上下文、知识、执行和记忆收拢成一个可靠的工作面。',
  ...
  channels: {
    ...
    webchatHint: 'Web / API 接入既可把 SnSclaw 聊天挂件嵌入外部网站，也可作为纯后端 SSE 接口直接调用。配置 API Key 后，前端可引入 WebChat SDK；后端可直接 POST /api/v1/channels/webchat/stream，头带 X-MC-Key、体传 {message, visitorId, 可选 sessionId/agentId}。',
  ...
  talk: {
    ...
    holdToTalk: '按住说话',
    releaseToSend: '松开发送',
    holdSpaceHint: '也可按住空格键说话，松开发送',
    holdingSpace: '正在录音，松开空格发送',
    retry: '点击重新连接',
    you: '你',
    ai: 'AI',
    micError: '无法访问麦克风',
