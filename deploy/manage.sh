...
#    ./manage.sh update server:v1.0.5   指定 tag 更新单个服务
...
# （server:v1.0.5-amd64 / server:v1.0.5-arm64），compose 用
...
cmd_update() {
    ...
    if [ -n "$target" ]; then
        # 形如 server:v1.0.5 —— 就地改 compose 里对应服务的 tag。
        # tag 不要带 -amd64/-arm64 后缀，架构由 IMAGE_ARCH_SUFFIX 自动补。
        local svc="${target%%:*}" tag="${target##*:}"
        [ "$svc" != "$tag" ] || die "格式应为 <服务>:<tag>，例如 server:v1.0.5"
        case "$tag" in
            *-amd64|*-arm64)
...
SnSclaw 运维脚本
  ...
  ./manage.sh update server:v1.0.5   改 tag 后再更新（自动校验 tag 是否存在）
...
多架构：镜像 tag 自动按宿主架构补 -amd64 / -arm64 后缀，
        ...
        update 指定 tag 时不要带架构后缀（写 server:v1.0.5）。
EOF
}
