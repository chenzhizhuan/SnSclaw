...
| 服务 | 镜像 | 宿主端口 | 容器端口 |
|---|---|---|---|
| snsclaw-server | `snsclaw/server:v1.0.5` | 19600 | 18088 |
| | | 1455 | 1455 |
...
./manage.sh update server:v1.0.5   # 改 tag 再更新，先校验 tag 存在性
...
| 仓库 | amd64 | arm64 |
|---|---|---|
| `snsclaw/server` | `v1.0.5-amd64` | `v1.0.5-arm64` |
...
`...:v1.0.5${IMAGE_ARCH_SUFFIX:-}`，拼接后即为本机架构对应的镜像。

无后缀的旧 tag（`server:v1.0.5` 等）仍保留在 registry 中指向 amd64 镜像，
供未升级脚本的老部署继续使用。
