...
if ($p.ExitCode -ne 0) { Append-Log $errL 60; Write-Tee '!! STEP1 FAIL'; exit 11 }
$idx = Join-Path $UI 'dist\index.html'
if (-not (Test-Path $idx)) { Write-Tee '!! STEP1 FAIL no dist\index.html'; exit 12 }
...
if (-not (Test-Path (Join-Path $STATIC 'index.html'))) { Write-Tee '!! STEP2 FAIL index.html not copied'; exit 21 }
