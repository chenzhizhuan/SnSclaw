...
$MASTER  = Join-Path $LOGS ('PIPELINE_RESUME2_' + [int][double](Get-Date -UFormat %s) + '.log')
...
Write-Tee "PIPELINE resume @ $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
Write-Tee "ROOT    = $ROOT"
Write-Tee "SRV     = $SRV"
Write-Tee "DSK     = $DSK"
...
if (-not (Test-Path (Join-Path $STATIC 'index.html'))) {
  Write-Tee '!! FAIL  static/index.html missing  (vite did not write there? need rebuild?)'
  exit 91
}
...
# ===== STEP 1: MVN =====
Write-Step 'STEP 1/4 MVN clean package (bakes static/ into BOOT-INF/classes/static/)'
$outL = Join-Path $LOGS 'r2-step1-mvn-out.log'
$errL = Join-Path $LOGS 'r2-step1-mvn-err.log'
$p = Run-Exe 'mvn.cmd' @('clean','package','-DskipTests','-Dmaven.test.skip=true') $SRV $outL $errL
if ($p.ExitCode -ne 0) { Append-Log $errL 80; Write-Tee '!! MVN FAIL'; exit 11 }
...
if (-not $JAR) { Write-Tee '!! MVN FAIL JAR missing'; exit 12 }
...
$outL = Join-Path $LOGS 'r2-step2-audit.log'
...
$outL = Join-Path $LOGS 'r2-step3-desktop-out.log'
$errL = Join-Path $LOGS 'r2-step3-desktop-err.log'
...
$outL2 = Join-Path $LOGS 'r2-step3-ws.log'
...
$outL = Join-Path $LOGS 'r2-step4-builder-out.log'
$errL = Join-Path $LOGS 'r2-step4-builder-err.log'
...
Write-Tee '  [X] Vite => server static/ DIRECT output (steps merged by vite config)'
